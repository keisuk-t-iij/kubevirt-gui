...
VirtualMachineのOS領域用ボリュームは以下の種類があります。VirtualMachineには必ず１つのOS領域用ボリュームを設定する必要があります。
...
VirtualMachineには任意で複数のデータ領域用ボリュームを設定することができます。
データ領域用ボリュームは以下の種類があります。
...
cloudInitNoCloudを利用する場合のVirtualMachineリソースは下記のようになります。
...
spec:
  ...
  template:
    spec:
      ...
      - name: cloud-init
        cloudInitNoCloud:
          userDataBase64: <ユーザーデータ>
          networkDataBase64: <ネットワークデータ>
```

### persistentVolumeClaim

persistentVolumeClaimを利用する場合のVirtualMachineリソースは下記のようになります。
...
configMapを利用する場合のVirtualMachineリソースは下記のようになります。
...
secretを利用する場合のVirtualMachineリソースは下記のようになります。
